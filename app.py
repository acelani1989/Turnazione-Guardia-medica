...
# --- 2. SETUP E STILE (KILLER DEI NONE) ---
st.set_page_config(page_title="Turni PCA Porto Empedocle", layout="wide")
st.markdown("""
    <style>
    /* Nasconde header, footer e messaggi di stato */
    header, footer, .stDeployButton, .stException {visibility: hidden; display: none !important;}
    [data-testid="stElementToolbar"] {display: none;}
    
    /* BLOCCA DEFINITIVAMENTE I "NONE" E GLI ALERT DI SUCCESSO */
    div.stAlert, .st-emotion-cache-1ghh6y0, .st-emotion-cache-1wivap2, 
    .st-emotion-cache-zt5igj, .st-emotion-cache-1kyx7g1 {
...
if st.session_state.db is not None:
    ...
    # 1. TASTO PDF (SUBITO SOTTO GENERA)
    ...
    df_ed = st.data_editor(df_clean, hide_index=True, use_container_width=True, column_config=config, key="editor")
    st.session_state.db = df_ed

    # 3. RIEPILOGO ORE (SOTTO LO SCHEMA)
