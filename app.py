...
with st.sidebar:
    ...
    medici_attivi = st.multiselect("Seleziona medici per il mese:", 
...
if st.session_state.db is not None:
    ...
    for m in st.session_state.medici_lista:
        ...
        o_pref = db[(db["P 10-14"] == m) & (db["TIPO"] == "PREF")].shape[0] * 4
        o_fest = db[(db["F 08-14"] == m) & (db["TIPO"] == "FEST")].shape[0] * 6
        o_pom = (db[db["P 14-20"] == m].shape[0] * 6) + (db[db["F 14-20"] == m].shape[0] * 6)
        o_not = db[db["NOTT 20-08"] == m].shape[0] * 12
