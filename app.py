...
if st.button("🚀 GENERA / RESETTA SCHEMA", type="primary", use_container_width=True):
...
if st.session_state.db is not None:
    ...
    df_ore = pd.DataFrame(riepilogo); tot = df_ore["O"].sum()
    ...
    with c2:
        if pdf_ok and st.button("📄 GENERA PDF", use_container_width=True):
            ...
            st.download_button("💾 SALVA PDF", pdf.output(dest='S').encode('latin-1'), f"Turni.pdf", "application/pdf", use_container_width=True)
