...
st.set_page_config(page_title="Turni Calvagna", layout="wide")
...
if st.button("🚀 GENERA SCHEMA", type="primary", use_container_width=True):
    ...
    for d in range(1, gg + 1):
        ...
        rows.append({
            "GIORNO": f"{d} {ita_g[wd]}", "PREF 10-14": "---", "PREF 14-20": "---", 
            "FEST 08-14": "---", "FEST 14-20": "---", "NOTT 20-08": "---",
...
if not st.session_state.db.empty:
    ...
    with pd.ExcelWriter(buffer_ex, engine='xlsxwriter') as writer:
        # Usiamo df_ed ma filtriamo solo le colonne visibili in modo sicuro
        cols_to_save = [c for c in df_ed.columns if c not in ["hM","hP","hN","TIPO"]]
        df_ex = df_ed[cols_to_save]
        df_ex.to_excel(writer, index=False, sheet_name='Turni')
        
        workbook = writer.book
        worksheet = writer.sheets['Turni']
        fmt_f = workbook.add_format({'bg_color': '#FFC7CE'}) # Rosso (Festivo)
        fmt_p = workbook.add_format({'bg_color': '#FFEB9C'}) # Giallo (Prefestivo)
    ...
    with col1:
        st.download_button("📥 SCARICA EXCEL COLORATO", buffer_ex.getvalue(), f"Turni_{mese_sel}.xlsx", use_container_width=True)

    with col2:
        if pdf_ok and st.button("📄 GENERA PDF (1 PAGINA)", use_container_width=True):
            pdf = FPDF('L', 'mm', 'A4')
            ...
            for _, r in df_ed.iterrows():
                if r["TIPO"] == "F": pdf.set_fill_color(255, 199, 206) # Rosso
                elif r["TIPO"] == "P": pdf.set_fill_color(255, 235, 156) # Giallo
                else: pdf.set_fill_color(255, 255, 255)
                
                pdf.cell(46, 6, str(r["GIORNO"]), 1, 0, 'L', True)
                pdf.cell(46, 6, str(r["PREF 10-14"]), 1, 0, 'C', True)
                pdf.cell(46, 6, str(r["PREF 14-20"]), 1, 0, 'C', True)
                pdf.cell(46, 6, str(r["FEST 08-14"]), 1, 0, 'C', True)
                pdf.cell(46, 6, str(r["FEST 14-20"]), 1, 0, 'C', True)
                pdf.cell(46, 6, str(r["NOTT 20-08"]), 1, 0, 'C', True)
                pdf.ln()
